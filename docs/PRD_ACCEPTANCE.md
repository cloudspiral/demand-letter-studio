...
| Review evidence before generation | Superseded by single-call grounded generation | Saved templates generate after source upload; new templates generate after map confirmation. Missing, ambiguous, or conflicting support produces one post-generation omission/null-field review item instead of a duplicate pre-generation model pass |
...
| Explicit attorney confirmation for unsupported content and attorney control of edits | Pass | Omitted targets require one versioned confirmation and missing fields require one saved value. Direct edits and accepted AI proposals are themselves approval and do not create a second confirmation task |
| Canonical server-computed export readiness | Pass | Draft reads and export share blockers derived only from unresolved omitted targets, null fields, stale evidence, and invalid/duplicate mappings |
...
| Attorney can further refine with AI | Pass | Exact annotations, streamed bounded proposal, visible tracked revisions, accept/reject, optimistic versioning, and activity history are covered; accepting the proposal is approval |
...
| Online collaboration/editing and change tracking (stretch) | Partial stretch | All existing generated/Keep/heading/header/footer text is directly editable; actor attribution, immutable version snapshots, Undo/Restore, stale-write rejection, proposals, and activity exist. Structural insertion/removal/reordering and simultaneous multi-user/CRDT editing are intentionally out of scope |
...
| PostgreSQL persistence preferred | Pass | Templates, sources/pages, generation jobs/events/dead letters, immutable draft versions/citations/omission confirmations, proposals/activity, and AI-run metadata persist in PostgreSQL 17. Historical evidence-review/resolution rows remain inert audit history |
