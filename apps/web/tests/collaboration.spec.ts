...
test("two signed users converge, reconnect, persist, and preserve agent proposals", async ({ browser }) => {
  ...
  await expect(faby.getByText("Draft editor")).toBeVisible({ timeout: 30_000 });
  await faby.getByRole("button", { name: "Live collaboration" }).click();
  ...
  const collaborativeText = "Shared settlement position: the documented medical charges total $42,750.00.";
  ...
  await faby.getByRole("button", { name: "Reject" }).click();
  await faby.reload();
  ...
  await expect(faby.getByText("Rejected an AI edit proposal")).toBeVisible();
