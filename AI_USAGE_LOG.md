...
|---|---|---|---|
| Evidence review | Configured OpenAI, Anthropic, Bedrock, or deterministic mock adapter | Matter label, confirmed editable template targets, replacement candidates, extracted page text, source/page IDs | Strict category-free JSON contract, exact source/page quote validation, stable normalized flag IDs, generic missing-support fallback for uncited claims, non-exhaustive advisory UI, no authenticity or legal-validity decision |
| Initial draft | OpenAI Responses / configured `gpt-5.6-sol` | Matter label, complete confirmed schema-v2 map, derived narrative/structured/figure targets, inline fields, current omission resolutions, extracted page text and allowed image inputs, source/page IDs | One whole-document call; `store: false`; strict JSON Schema and Zod parse; exact target/field coverage; paragraph/row limits; citation/source/page/media validation; previous-matter leakage, conflict, resolution-fingerprint, and export gates |
...
| Refinement | Same configured provider chain | Instruction, up to five exact block/range annotations, page-aware evidence | Immediate SSE status, bounded edit proposal, exact target validation, explicit accept/reject, atomic version check |
...
Models never receive API keys, execute code, access the filesystem, or generate OOXML. Prompts and document contents are not written to application logs. The Python worker, not the model, performs deterministic package updates. Review flags explain possible evidence issues but do not classify documents, authenticate records, or decide whether a legal position is valid; deterministic grounding, explicit attorney confirmation, template coverage, and server-computed export readiness remain authoritative.
