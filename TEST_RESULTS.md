...
- `pnpm verify` passed with 5 contract, 56 API, 2 web-client, and 18 Python worker tests (81 total), followed by production API and web builds. Vite reported only its existing large-chunk advisory.
- Three routed Playwright tests passed for template-card containment, the synchronized new-template map, and the compact generated Review workspace at 1280x720 and 1100x720. The map test now confirms that a new template starts exactly one generation immediately after map confirmation. The workspace test covers exactly one card per unresolved omission/field, omission confirmation, field correction, direct Keep-text editing, persisted Undo, sticky matter rename, and version history.
- One isolated full PostgreSQL Playwright workflow passed with the deterministic provider in 6.1 seconds. It covers upload, map confirmation or saved-template start, direct whole-context generation without evidence review, source citations, stale-evidence regeneration, omission/field remediation, direct editing, AI acceptance without a second confirmation, activity, and Word download.
- Contract and generation tests cover the two target statuses, nullable field results, conditional notes/citations, missing/conflicting evidence, cited negative facts, exact target/field coverage, malformed and non-verbatim citation rejection, figure and structured constraints, source fingerprints, and previous-case leakage prevention.
- API and worker tests cover neutral and grounded automatic naming, sticky manual rename, legacy read normalization, snapshot-scoped omission confirmation, orphan-heading blanking with anchor preservation, editable Keep/headings, optimistic restore, readiness parity, and deterministic export of restored/edited content without changing unrelated OOXML parts.
- The final authorized local OpenAI browser workflow passed in 2.1 minutes with provider storage disabled. Its first whole-context generation attempt completed in 112.919 seconds, returned 9/9 target outcomes and 17/17 field outcomes, and persisted 5 generated plus 4 omitted targets. Browser remediation confirmed all 4 omissions and supplied every missing field, then saved a direct edit, rejected a stale write, completed and accepted a 7.036-second OpenAI refinement without another confirmation, advanced the draft to v17, and downloaded Word successfully.
- Three earlier diagnostic runs were not counted as acceptance passes: one hit the old 180-second browser timeout while still processing; one failed closed when OpenAI reused an unsupported previous-case value and Anthropic returned malformed JSON; one failed closed on a non-verbatim OpenAI citation while the Anthropic fallback returned no text. The repair removed model-authored titles, supplied an explicit forbidden-value list, retained exact leakage/citation validation, and added one bounded whole-context primary retry. The final passing run used its first attempt.
- The live export was independently reopened as a ZIP and rendered through headless LibreOffice. Stable `w14:paraId` alignment found no paragraph-formatting or special-OOXML drift; only `word/document.xml`, `word/header1.xml`, and the explicitly allowed `word/media/image1.png` changed. Four confirmed omissions reduced the body from 152 to 149 paragraphs and the render from six to five pages; every candidate page remained 612x792-point US letter. The aligned report passed with those two intentional count-change warnings and no failures at `/private/tmp/steno-single-call-docx-report-aligned.json`.

The redesign deliberately removes the evidence-review endpoint/job/provider call, model-returned warning and review-flag taxonomies, pre-generation omission controls, the block-confirmation endpoint, and new writes to the legacy resolution schema. Existing historical rows and old draft snapshots are retained; legacy omission statuses and approvals are normalized only when read.
